...
export async function wdc(client, message, args, data) {
  try {
    ...
    if (args.length === 0) {
      const embed = baseEmbed()
        .setTitle('❌ Erreur')
        .setDescription('Utilisation: `=wdc @utilisateur`');
      return message.reply({ embeds: [embed] });
    }
    ...
    if (!data.protect.valid.DECO) data.protect.valid.DECO = [];

    // Vérifier si l'utilisateur est wall (ajout automatique)
    const isWall = data.wall && data.wall.includes(targetId);
    const isAlreadyValid = data.protect.valid.DECO.includes(targetId);

    if (isWall && !isAlreadyValid) {
      data.protect.valid.DECO.push(targetId);
      safeWriteJson(DATA_PATH, data);

      const embed = baseEmbed()
        .setTitle('✅ Validé automatiquement (Wall)')
        .setDescription(`**${targetMember.displayName}** a été ajouté à la liste DECO (utilisateur wall).`);
      return message.reply({ embeds: [embed] });
    }

    if (isAlreadyValid) {
      // Retirer de la liste
      data.protect.valid.DECO = data.protect.valid.DECO.filter(id => id !== targetId);
      safeWriteJson(DATA_PATH, data);

      const embed = baseEmbed()
        .setTitle('✅ Retiré de DECO')
        .setDescription(`**${targetMember.displayName}** a été retiré de la liste DECO.`);
      ...
      const embed = baseEmbed()
        .setTitle('✅ Ajouté à DECO')
        .setDescription(`**${targetMember.displayName}** a été ajouté à la liste DECO.`);
      message.reply({ embeds: [embed] });
    }

  } catch (error) {
    console.error('Erreur wdc:', error);
    const embed = baseEmbed()
      .setTitle('❌ Erreur')
